...
class FrameComposer(
    ...
    private fun makeOverlay(cue: SubtitleCue?): Bitmap {
        ...
        if (cue != null) {
            val paint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
                typeface = typefaceFor(options.fontName)
    ...
    private fun typefaceFor(name: String): Typeface = when (name) {
        "Elegant English" -> Typeface.create("serif", Typeface.BOLD)
        "Cinematic Serif" -> Typeface.create("serif", Typeface.NORMAL)
        "Movie Sans" -> Typeface.create("sans-serif-condensed", Typeface.NORMAL)
        "Classic Arabic" -> Typeface.create("sans-serif", Typeface.NORMAL)
